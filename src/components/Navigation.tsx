    ...
    <>
      <nav
        className={cn(
          ...
          isScrolled
            ? "bg-paper-cream/90 backdrop-blur-lg border-b border-line py-4 shadow-sm"
            : "bg-transparent border-b border-transparent"
        )}
      >
        <a
          href="#top"
          className="font-disp text-xl font-semibold tracking-[0.28em] uppercase text-ink no-underline select-none"
        >
          El Guapo
        </a>
        ...
        <ul className="hidden lg:flex gap-10">
          {links.map((link) => (
            <li key={link.name}>
              <a
                href={link.href}
                className="text-[0.67rem] tracking-[0.22em] uppercase text-ink-soft hover:text-terra transition-colors relative group"
              >
                {link.name}
        ...
        {/* Mobile Hamburger */}
        <button
          className="lg:hidden p-2 -mr-2 text-ink-soft hover:text-terra transition-colors focus:outline-none"
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-12">
                ...
                <button
                  onClick={() => setIsDrawerOpen(false)}
                  className="text-ink-soft hover:text-terra transition-colors"
                >
                ...
                {links.map((link, i) => (
                  ...
                  >
                    <a
                      href={link.href}
                      onClick={() => setIsDrawerOpen(false)}
                      className="flex items-center gap-4 text-2xl font-disp italic text-ink hover:text-terra transition-colors"
                    >
                      <span className="text-sm font-body not-italic text-line font-light">
                        0{i + 1}
                      </span>
                      {link.name}
                    </a>
